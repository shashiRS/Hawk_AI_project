...
class HyderbaddataSpider(scrapy.Spider):
    name = 'hyderbaddata'
...
"telangana"]
        ...
        for i in range(1,5):
            ...
            for loc in self.localit:
                print loc
                url='https://www.magicbricks.com/Real-estate-projects-Search/residential-new-project?proptype=Multistorey-Apartment,Penthouse,Studio-Apartment,Service-Apartment,Residential-House,Villa,Residential-Plot&Locality=%s&cityName=Hyderabad&price=Y&page=%s&bar_propertyType_new=10002_10021_10022_10020,10001_10017,10000&mbTrackSrc=tabChange&tab%sProject=property'%(loc,i,j)
                self.start_url.append(url)
               
           
            print("----------------------------------------------------------------------------------------------------------------------------------------------------------------")
            super(HyderbaddataSpider, self).__init__(*args, **kwargs)
