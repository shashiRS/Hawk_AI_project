...
class GoogleTrendsScraper:
    ...
    def generate_url(self, start_date, end_date):
        ...
        date = "date={start_date}%20{end_date}"
    ...
    def fetch_week_trends(self, url, output_file_name=original_output_file_name):
        ...
        driver = webdriver.Firefox(p, executable_path="google_trends_scraper/geckodriver")
        driver.get(url)
        ...
        print("about to rename {self.original_output_file_name} to {output_file_name}")
    ...
    def combine_csv_files(self, file_names, output=None):
        ...
        dfs = []
        for filename in sorted(ile_names):
            dfs.append(pd.read_csv(ilename, skiprows=2))
    ...
    def weekly_scrape(self):
        ...
        for i in range(0, len(weeks), 1):
            ...
            self.fetch_week_trends(url, "{start_day}_to_{end_day}.csv")

            print("Waiting {self.seconds_delay} to avoid IP banning")
    ...
    def total_scrape(self):
        ...
        self.fetch_week_trends(url, "{self.start_date}_to_{self.end_date}.csv")

        return pd.read_csv("{self.start_date}_to_{self.end_date}.csv")
